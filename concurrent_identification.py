...
def concurrent_identification(dataset,models,id_params,use_bounds = True, zeta = 1.05, tightened_W = False, k_max = 5):
    ...
    def RCI_computation_single(params, yRCI_prev, factor=1):
        ...
        Q = 2*RR.T @ RR
        num_vars = Q.shape[0]
        Q = Q+regularization_QP*np.eye(num_vars)
        c = -2*RR.T@yvec_kron
    ...
    x0_guess = predict_initial_states(model_qLPV, Us_train, Ys_train, activation=activation)
    ...
    for i in range(adam_epochs):
        ...
        str = f"    Curr = {sysID_loss: 10.6f}, Best = {best_cost_history[-1]: 8.6f}, RCIb = {cost_RCI_best: 2.3f}, a_new = {a_rounded}, W = {w_hat_best[0]: 2.3f}, {epsw_best[0]: 2.3f}, ||| costs : RCIc = {weight_RCI*cost_RCI_curr: 8.6f}, loss = {loss_prediction_best: 8.6f}"
